...
try:
    if args[0] == '-p' and '&list=' in args[1]:
        ...
        get_json = f"yt-dlp --no-warnings --no-abort-on-error --dump-single-json {args[1]}"
        get_json = get_json.split(' ')
        print('Getting JSON information from playlist. Could take a while')
        jayson = subprocess.check_output(get_json,stderr=subprocess.DEVNULL)
        with open('jayson.json','w') as f:
            json.dump(jayson.decode('utf-8'), f)
            #data = json.load(jayson.decode('utf-8'))
        ...
        #cmd = f"yt-dlp -x -f bestaudio -o {new_path}/%(title)s.%(ext)s {args[1]}"
        #cmd = cmd.split(' ')
