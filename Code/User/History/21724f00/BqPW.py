...
try:
    if args[0] == '-p' and '&list=' in args[1]:
        ...
        get_json = f"yt-dlp --flat-playlist --skip-download -o %(playlist_title)s {args[1]}"
        get_json = get_json.split(' ')
        print('Getting JSON information from playlist. Could take a while')
        jayson = subprocess.check_output(get_json).decode().split('Finished downloading playlist: ')[1].split('\n')[0]
        print(jayson)
